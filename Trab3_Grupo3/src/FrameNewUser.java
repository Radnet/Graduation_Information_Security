...
public class FrameNewUser extends JFrame {
    ...
    public FrameNewUser(String Title) {
        ...
        JLabel LB_userForm = new JLabel("Formul�rio de Cadastro");
  		//************************************************
        ...
        JLabel LB_UserPswConf = new JLabel("Confirma��o de senha:");
    ...
    public boolean IsAllFieldsOK() {
        ...
        //Verify sequence at start
        if(psw.charAt(i + 1) - psw.charAt(i) == 1 && psw.charAt(i) - psw.charAt(i - 1) == 1
        || psw.charAt(i) == psw.charAt(i - 1)){
            ErrorMessage = "A senha nao pode conter nenhum tipo de sequencia ou repeticao";
            return false;
        }
        
        //Verify sequences in middle
        for (i = 2; i < psw.length() - 1; i++) 
        {
            if (Math.abs(psw.charAt(i) - psw.charAt(i - 1)) == 1 
                        && Math.abs(psw.charAt(i - 1) - psw.charAt(i - 2)) == 1 
                        || psw.charAt(i) == psw.charAt(i - 1)) 
            {
                ErrorMessage = "A senha nao pode conter nenhum tipo de sequencia ou repeticao";
                return false;
        ...
        //Verify sequence at end
        if(psw.charAt(i) - psw.charAt(i - 1) == 1 && psw.charAt(i - 1) - psw.charAt(i - 2) == 1            
            || psw.charAt(i) == psw.charAt(i - 1)){
            ErrorMessage = "A senha nao pode conter nenhum tipo de sequencia ou repeticao";
            return false;
        }
